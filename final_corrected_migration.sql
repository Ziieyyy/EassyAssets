-- Final Corrected Categories Migration Script for Supabase SQL Editor
...
-- Check if the constraint exists and create appropriate index
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_unique_user_category_name') THEN
        -- Check if any similar constraint exists and drop if needed
...
-- Use DISTINCT ON to avoid duplicates based on user_id and category name combination
INSERT INTO categories (user_id, name, is_active)
SELECT DISTINCT ON (a.user_id, a.category) a.user_id, a.category, TRUE
FROM assets a
...
ORDER BY a.user_id, a.category, a.created_at;
